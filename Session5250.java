...
public class Session5250 implements SessionInterface {
	...
	private final tn5250j.SessionConfig sesConfig;
	private tnvt vt;
	private final Screen5250 screen;
	private tn5250j.SessionPanel guiComponent;
	...
	public Session5250 (Properties props, String configurationResource,
		...
		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_HEART_BEAT))
			heartBeat = true;
	...
	public boolean isSslConfigured() {
		if (sesProps.get(tn5250j.TN5250jConstants.SSL_TYPE) != null) {
			final String sslType = (String) sesProps.get(tn5250j.TN5250jConstants.SSL_TYPE);
			if (!tn5250j.TN5250jConstants.SSL_TYPE_NONE.equals(sslType)) {
				return true;
			}
		}
		return false;
	...
	public boolean isUseSystemName() {
		return sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_TERM_NAME_SYSTEM) != null;
	}
	...
	public void setGUI (tn5250j.SessionPanel gui) {
		guiComponent = gui;
	}
	...
	public void connect() {
		...
		enhanced = sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_TN_ENHANCED);

		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_SCREEN_SIZE))
			if ((sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_SCREEN_SIZE)).equals(tn5250j.TN5250jConstants.SCREEN_SIZE_27X132_STR))
				support132 = true;
		...
		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_PROXY_PORT))
			proxyPort = sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_PROXY_PORT);

		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_PROXY_HOST))
			vt.setProxy(sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_PROXY_HOST),
					proxyPort);

		final String sslType;
		if (sesProps.containsKey(tn5250j.TN5250jConstants.SSL_TYPE)) {
			sslType = sesProps.getProperty(tn5250j.TN5250jConstants.SSL_TYPE);
		} else {
			// set default to none
			sslType = tn5250j.TN5250jConstants.SSL_TYPE_NONE;
		}
		vt.setSSLType(sslType);

		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_CODE_PAGE))
			vt.setCodePage(sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_CODE_PAGE));

		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_DEVICE_NAME))
			vt.setDeviceName(sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_DEVICE_NAME));

		if (sesProps.containsKey(tn5250j.TN5250jConstants.SESSION_HOST_PORT)) {
			port = Integer.parseInt(sesProps.getProperty(tn5250j.TN5250jConstants.SESSION_HOST_PORT));
		}
		else {
