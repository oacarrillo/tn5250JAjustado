...
import tn5250j.BootStrapper;
import tn5250j.ConnectDialog;
import tn5250j.Gui5250Frame;
...
//		startFrameType();
	...
	public void bootOptionsReceived(BootEvent bootEvent) {
		...
		if (!bootEvent.getNewSessionOptions().equals("null")) {
			...
			String[] args = new String[tn5250j.TN5250jConstants.NUM_PARMS];
...
//		}
		...
		if (m.sessionArgs == null && sessions.containsKey("emul.view") &&
			...
			String[] sargs = new String[tn5250j.TN5250jConstants.NUM_PARMS];
		...
		if (m.sessionArgs != null) {
			...
			for (int x = 1; x < os400_sessions.size(); x++ ) {
				...
				m.sessionArgs = new String[tn5250j.TN5250jConstants.NUM_PARMS];
...
//	}
	...
	private void startNewSession() {
		...
		if (sel != null && sess.getCount() == 0
			...
			sessionArgs = new String[tn5250j.TN5250jConstants.NUM_PARMS];
	...
	private synchronized void newSession(String sel,String[] args) {
		...
		sesProps.put(tn5250j.TN5250jConstants.SESSION_HOST,session);

		if (isSpecified("-e",args))
			sesProps.put(tn5250j.TN5250jConstants.SESSION_TN_ENHANCED,"1");

		if (isSpecified("-p",args)) {
			sesProps.put(tn5250j.TN5250jConstants.SESSION_HOST_PORT,getParm("-p",args));
		}
		...
		if (isSpecified("-cp",args))
			sesProps.put(tn5250j.TN5250jConstants.SESSION_CODE_PAGE ,getParm("-cp",args));

		if (isSpecified("-gui",args))
			sesProps.put(tn5250j.TN5250jConstants.SESSION_USE_GUI,"1");

		if (isSpecified("-t", args))
			sesProps.put(tn5250j.TN5250jConstants.SESSION_TERM_NAME_SYSTEM, "1");

		if (isSpecified("-132",args))
			sesProps.put(tn5250j.TN5250jConstants.SESSION_SCREEN_SIZE, tn5250j.TN5250jConstants.SCREEN_SIZE_27X132_STR);
		else
			sesProps.put(tn5250j.TN5250jConstants.SESSION_SCREEN_SIZE, tn5250j.TN5250jConstants.SCREEN_SIZE_24X80_STR);
		...
		if (isSpecified("-usp",args)) {
			...
			if (isSpecified("-sph",args)) {
				sesProps.put(tn5250j.TN5250jConstants.SESSION_PROXY_HOST ,getParm("-sph",args));
			}

			// socks proxy port argument
			if (isSpecified("-spp",args))
				sesProps.put(tn5250j.TN5250jConstants.SESSION_PROXY_PORT ,getParm("-spp",args));
		}

		// are we to use a ssl and if we are what type
		if (isSpecified("-sslType",args)) {

			sesProps.put(tn5250j.TN5250jConstants.SSL_TYPE,getParm("-sslType",args));
		}
		...
		if (isSpecified("-dn=hostname",args)){
			...
			sesProps.put(tn5250j.TN5250jConstants.SESSION_DEVICE_NAME ,dnParam);
		}
		else if (isSpecified("-dn",args)){

			sesProps.put(tn5250j.TN5250jConstants.SESSION_DEVICE_NAME ,getParm("-dn",args));
		}

		if (isSpecified("-hb",args))
			sesProps.put(tn5250j.TN5250jConstants.SESSION_HEART_BEAT,"1");
