...
public class My5250Applet extends JApplet {
   ...
   private void jbInit() throws Exception {
     ...
     log = TN5250jLogFactory.getLogger (this.getClass());
      ...
      sesProps.put(tn5250j.TN5250jConstants.SESSION_HOST,getParameter("host"));

      if (isSpecified("-e"))
         sesProps.put(tn5250j.TN5250jConstants.SESSION_TN_ENHANCED,"1");

      if (isSpecified("-p")) {
         sesProps.put(tn5250j.TN5250jConstants.SESSION_HOST_PORT,getParameter("-p"));
      }
...
//         propFileName = getParm("-f",args);
      ...
      if (isSpecified("-cp"))
         sesProps.put(tn5250j.TN5250jConstants.SESSION_CODE_PAGE ,getParameter("-cp"));
      ...
      if (isSpecified("-sph")) {
         sesProps.put(tn5250j.TN5250jConstants.SESSION_PROXY_HOST ,getParameter("-sph"));
      }

      // socks proxy port argument
      if (isSpecified("-spp"))
         sesProps.put(tn5250j.TN5250jConstants.SESSION_PROXY_PORT ,getParameter("-spp"));

      // check if device name is specified
      if (isSpecified("-dn"))
         sesProps.put(tn5250j.TN5250jConstants.SESSION_DEVICE_NAME ,getParameter("-dn"));
      // are we to use a ssl and if we are what type

      if (isSpecified("-sslType")) {

         sesProps.put(tn5250j.TN5250jConstants.SSL_TYPE,getParameter("-sslType"));
      }
      ...
      final tn5250j.SessionPanel gui = new SessionPanel(s);
...
//            jt.grabFocus();
   ...
   public String getAppletInfo() {
      return "tn5250j - " + tn5250j.TN5250jConstants.tn5250jRelease + tn5250j.TN5250jConstants.tn5250jVersion + TN5250jConstants.tn5250jSubVer + " - Jave tn5250 Client";
   }
