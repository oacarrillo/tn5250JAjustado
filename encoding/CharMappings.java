...
public class CharMappings {
	...
	public static String[] getAvailableCodePages() {
		...
		for (String cp : tn5250j.encoding.BuiltInCodePageFactory.getInstance().getAvailableCodePages()) {
			cpset.add(cp);
		}
		for (String cp : tn5250j.encoding.ToolboxCodePageFactory.getInstance().getAvailableCodePages()) {
			cpset.add(cp);
		}
	...
	public static ICodePage getCodePage(String encoding) {
		...
		ICodePage cp = tn5250j.encoding.BuiltInCodePageFactory.getInstance().getCodePage(encoding);
		if (cp != null) {
			map.put(encoding, cp);
			return cp;
		}

		cp = tn5250j.encoding.ToolboxCodePageFactory.getInstance().getCodePage(encoding);
		if (cp != null) {
			map.put(encoding, cp);
			return cp;
		}

		cp = tn5250j.encoding.JavaCodePageFactory.getCodePage(encoding);
		if (cp != null) {
			map.put(encoding, cp);
			return cp;
		}
		...
		return tn5250j.encoding.BuiltInCodePageFactory.getInstance().getCodePage(DFT_ENC);
	}
